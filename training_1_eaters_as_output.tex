This subsection will describe a method of optimizing a \textsc{Nenwin} architecture with a given set of Nodes to give the desired output. 
...
% To begin, note that by definition, we can rewrite the element at each index $i$ of $\hat{\vec{y}}$ as:
% \begin{align}
%     \hat{y}_i & \equiv \norm{\{m | \exists_t \left[t \in [t_0, t_{end}] : \norm{m.pos(t) - E_i.pos(t)} \leq E_i.radius\right]\}} \\
%     & = \sum_{t \in [t_0, t_{end}]} \mathbbm{1}(\norm{m.pos(t) - E_i.pos(t)} \leq E_i.radius)
% \end{align}
% Where $E_i$ is the $i^{th}$ MarbleEaterNode of the model.
...
\subsubsection{Backpropagation of mass-gradient through MarbleEmitterNodes}
...
\subsubsection{Non-propagatable gradients}
...
\subsubsection{Backpropagation discussion}
...
TODO:
\begin{enumerate}
    \item add formula for stored\_mass
\end{enumerate}
}